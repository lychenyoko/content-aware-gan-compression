...
class LBFGS(Optimizer):
    ...
    def _step(self, p_k, g_Ok, g_Sk=None, options={}, device=None):
        """
            ...
            if(interpolate):
                if(torch.cuda.is_available()):
                    if device is None:
                        ...
                        F_prev = torch.tensor(np.nan, dtype=dtype).to(device)
                else:
            ...
            if(interpolate):
                ...
                if(torch.cuda.is_available()):
                    if device is None:
                        ...
                        F_b = torch.tensor(np.nan, dtype=dtype).to(device)
                        g_b = torch.tensor(np.nan, dtype=dtype).to(device)
                else:
...
class FullBatchLBFGS(LBFGS):
    ...
    def step(self, options={}, device=None):
        """
        Performs a single optimization step.
        ...
        # take step
        return self._step(p, grad, options=options, device=device)
